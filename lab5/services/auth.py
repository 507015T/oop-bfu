...
class AuthService(AuthServiceProtocol):
    def __init__(self, session_file: str = "session.json", repo: UserRepository = None):
        ...
        if self.session_file.exists():
            try:
                with open(self.session_file, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    if self._repo:
                        user = self._repo.get_by_id(data["id"])
                        self._current_user = user
                    else:
                        logging.error(
                            "UserRepository not provided. Cannot load session."
                        )
    ...
    def _save_session(self):
        if self._current_user:
            try:
                ...
                logging.error(
                    f"Error saving session file: {e}, current user id: {self._current_user.id}"
                )
